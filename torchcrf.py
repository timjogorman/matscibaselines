...
class CRF(nn.Module):
    ...
    def __init__(
        ...
        self.trans_matrix = torch.cuda.FloatTensor(torch.empty(num_labels, num_labels).cuda())
        # transition matrix of start and end settings
        self.start_trans = torch.cuda.FloatTensor(torch.empty(num_labels).cuda())
        self.end_trans = torch.cuda.FloatTensor(torch.empty(num_labels).cuda())
    ...
    def _initialize_parameters(self, pad_idx: Optional[int]) -> None:
        ...
        if self._use_gpu:
            self.start_trans = self.start_trans.cuda()
